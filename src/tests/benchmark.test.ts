...
function benchmark(name: string, fn: () => void, iterations: number = 100) {
  ...
  console.log(`${name}:`);
  console.log(`  Total: ${total.toFixed(2)}ms (${iterations} iterations)`);
  console.log(`  Average: ${avg.toFixed(3)}ms per iteration`);
  return { total, avg, iterations };
}

describe('BENCHMARK: simplifyDebts performance', () => {
  it('profiles real graph performance', () => {
    console.log('\n=== PROFILING simplifyDebts (BEFORE OPTIMIZATION) ===\n');
    ...
    expect(realResult.avg).toBeLessThan(1000); // Should be fast enough
  });
});
